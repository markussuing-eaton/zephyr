...
static int tlv_mgmt_post_recv(struct ptp_tlv_mgmt **pMgmt_tlv, uint16_t length)
{
	...
	struct ptp_tlv_mgmt *mgmt_tlv = *pMgmt_tlv;
	...
	case PTP_MGMT_CLOCK_DESCRIPTION:
	    // see issue #99527
		container = CONTAINER_OF((void *)pMgmt_tlv, struct ptp_tlv_container, tlv);
...
int ptp_tlv_post_recv(struct ptp_tlv **pTlv)
{
	struct ptp_tlv_mgmt_err *mgmt_err;
	struct ptp_tlv_mgmt *mgmt;
	int ret = 0;
    struct ptp_tlv *tlv = *pTlv;
	...
	case PTP_TLV_TYPE_MANAGEMENT:
		...
		if (tlv->length > sizeof(mgmt->id)) {
			ret = tlv_mgmt_post_recv((struct ptp_tlv_mgmt **)pTlv, tlv->length - 2);
		}
		break;
